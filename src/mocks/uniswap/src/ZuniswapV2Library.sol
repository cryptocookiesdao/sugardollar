...
library ZuniswapV2Library {
        ...
        returns (uint256 reserveA, uint256 reserveB)
    {
        ...
        emit TT("TOKEN", pairFor(factoryAddress, token0, token1));
        (uint256 reserve0, uint256 reserve1,) = IZuniswapV2Pair(pairFor(factoryAddress, token0, token1)).getReserves();
    ...
    {
        (address token0, address token1) = sortTokens(tokenA, tokenB);
        pairAddress = IZuniswapV2Factory(factoryAddress).getPair(token0, token1);
        /*
        pairAddress = address(
            uint160(
                uint256(
