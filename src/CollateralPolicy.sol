...
contract CollateralPolicy {
    ...
    function updateAndGet() external returns (uint256) {
        ...
        uint256 priceDollar = oracle.sugarDollarPrice();
        ...
        unchecked {
            if (priceDollar < BAND_BOTTOM && _targetCollateral < MAX_TARGET) {
                ...
                _targetCollateral = _targetCollateral > MAX_TARGET
                    ? MAX_TARGET
                    : _targetCollateral;
            } else if (
                priceDollar > BAND_TOP && MIN_TARGET < _targetCollateral
            ) {
                // decrease TCR only if price is higher than BAND_TOP and havent reach MIN_TARGET
                _targetCollateral -= TARGET_STEP;
                _targetCollateral = _targetCollateral < MIN_TARGET
                    ? MIN_TARGET
                    : _targetCollateral;
            }
        }
